package com.juntu.androidlib.imgcompress.checker
...
enum class Checker {
    ...
    private fun getOrientation(jpeg: ByteArray?): Int {
        ...
        while (offset + 3 < jpeg.size && (jpeg[offset++] and 0xFF.toByte()) == 0xFF.toByte()) {
            val marker = (jpeg[offset] and 0xFF.toByte()).toInt()
    ...
    open fun extSuffix(input: InputStreamProvider): String? {
        return try {
     ...
     * 文件小于某个值就不进行压缩
     */
    open fun needCompress(compressSize: Int, path: String?): Boolean {
        if (compressSize > 0) {
            val source = File(path)
            return source.exists() && source.length() > (compressSize shl 10)
        }
        return true
    }
